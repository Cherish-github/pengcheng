define(['jquery', 'admin.plugs'], function () {
    ...
    this.$body.on('click', '[data-file]', function () {
        ...
        var limit = $(this).attr('data-limit') || '300';
        var uptype = $(this).attr('data-uptype') || '';
        if(method=="mtl"&& limit==="0"){
            return $.msg.tips('文件上传达到上限！');
        }
        var url = window.ROOT_URL + '/index.php/admin/plugs/upfile/mode/' + method + '.html?mode=' + method + '&uptype=' + uptype + '&type=' + type + '&field=' + field+"&limit="+limit;
        $.form.iframe(url, title || '文件管理');
    });


    /*! 注册 data-file 事件行为 */
    this.$body.on('click', '[data-crop]', function () {
        var scale   = $(this).attr('data-scale') || "";
        var elem   = $(this).attr('data-elem');
        if(!elem) return layer.msg('参数设置错误~');
        var url = window.ROOT_URL + '/index.php/admin/plugs/cropImage?scale='+scale+'&elem='+elem;
        return layer.open({title:'裁切上传', type: 2, area: ['800px', '80%'], fix: true, maxmin: false, content: url});
    });
    ...
    /*注册异步刷新 排序*/
    this.$body.on('click', '[data-sort]', function () {
        var action = $(this).attr('data-action') || $(this).parents('[data-location]').attr('data-location');
        var value = $(this).attr('data-value') || 0, field = $(this).attr('data-field') || '0';
        if(!value || !field){
            return false;
        }
        $.form.load(action, {field: field, value: value}, 'get');
    });
